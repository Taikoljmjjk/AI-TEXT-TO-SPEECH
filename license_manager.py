"""Xác minh license TAILEMMO offline bằng chữ ký Ed25519 và mã máy Windows.

Module phân phối chỉ chứa public key. Private seed luôn nằm trong công cụ admin.
"""
...
APP_DIR = Path(os.getenv("APPDATA", Path.home())) / "TAILEMMO Voice 11 Labs"
...
def _point_add(point_p, point_q):
    x1, y1 = point_p
    x2, y2 = point_q
...
def _read_last_seen() -> int:
    try:
        data = json.loads(_unprotect(CLOCK_FILE.read_text(encoding="ascii")))
        return int(data["last_seen"])
    except Exception:
        return 0
...
def _write_last_seen(timestamp: int) -> None:
    ...
    encoded = _protect(json.dumps({"last_seen": int(timestamp)}, separators=(",", ":")))
    temporary = CLOCK_FILE.with_suffix(".tmp")
    temporary.write_text(encoded, encoding="ascii")
    temporary.replace(CLOCK_FILE)
...
def verify_license(
    ...
    *,
    ...
    if not _verify_signature(signature, payload_raw, _b64decode(PUBLIC_KEY_B64)):
    ...
    if not permanent and expires_at <= issued_at:
    ...
    if not permanent and current_time >= expires_at:
...
def save_license(key_text: str) -> None:
    APP_DIR.mkdir(parents=True, exist_ok=True)
    encrypted = _protect("".join(key_text.split()))
...
def load_license() -> str:
    try:
        return _unprotect(LICENSE_FILE.read_text(encoding="ascii")).strip()
    except Exception:
        return ""
...
def remaining_seconds(payload: dict, now: int | None = None) -> int:
    ...
    return max(0, int(payload.get("expires_at", 0)) - current)
